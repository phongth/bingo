...
public class GameActionHandle implements DataReceiveListener, AlertListener {
	...
	public void alertEventPerform(int alertType, int eventType, int alertId) {
		// Nếu đăng nhập vào GameService thất bại thì cho out ra màn hình đăng
		// nhập để đăng nhập lại
		if (alertId == 99) {
	...
	public void onRecieveData(DataPackage dataPackage) {
		...
		case ProtocolConstants.ResponseHeader.SALT_REPONSE:
			...
			SocketClientUtil.authenToGameServerBySessionId(Global.loginInfo
					.getUserName(), Global.session.getId(), salt);
			break;
		...
		case ProtocolConstants.ResponseHeader.AUTHENTICATE_FAIL_RESPONSE:
			GameGlobal.alert.showAlert(this, Alert.OK_TYPE, new String[] {
					"Đăng nhập vào server game thất bại",
					"Hãy thử đăng nhập lại" }, 99);
			break;
	...
	private static DataPackage getGameActionPackage(DataPackage dataPackage) {
		try {
			dataPackage.setHeader(dataPackage.nextInt()); // Lấy data thứ 2 làm
															// header
		} catch (ArrayIndexOutOfBoundsException ex) {
			System.err
					.println("ERROR : GameActionHandle : getGameActionPackage : dataPackage has wrong format");
		}
		return dataPackage;
	}
	...
	public void onConnectDone() {
		SocketClientUtil
				.startAuthenToGameServer(Global.loginInfo.getUserName());
	}

	public void onConnectFail() {
		GameGlobal.alert.showAlert(this, Alert.OK_TYPE,
				"Kết nối đến server game thất bại");
	}

	public void onDisconnect() {
		if (!isOnSignOut) {
			Alert.instance().showAlert(this, Alert.OK_TYPE,
					"Mất kết nối với máy chủ", 96);
		}
		isOnSignOut = false;
	}
	...
	public void onAuthenSuccess() {
		...
		if (Global.authenClient != null) {
			Global.authenClient
					.write(new DataPackage(
							DefaultProtocolConstants.RequestHeader.CLOSE_CONNECTION_REQUEST));
