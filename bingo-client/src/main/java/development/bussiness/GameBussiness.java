...
public class GameBussiness implements AlertListener {
	public void alertEventPerform(int alertType, int eventType, int alertId) {
		// Khi user không đủ tiền so với đặt cược mới, thì thông báo và thoát ra
		// danh sách bàn
		if (alertId == 98) {
		...
		// Khi người điểm cược thay đổi và người chơi đủ tiền, hỏi người chơi là
	...
	public void onListUsersWantToPlay(DataPackage dataPackage) {
		int pageNumber = dataPackage.nextInt();
	...
	public void onStartGame() {
		if (Global.currentGame.getId().equals("caro")) {
			GameGlobal.nextState(Global.frmCaro, null,
					Transformer.TRANSFORM_WITH_LOADING_FORM);
		} else if (Global.currentGame.getId().equals("cotuong")) {
			GameGlobal.nextState(Global.frmChess, null,
					Transformer.TRANSFORM_WITH_LOADING_FORM);
		} else if (Global.currentGame.getId().equals("covua")) {
			GameGlobal.nextState(Global.frmBroadChess, null,
					Transformer.TRANSFORM_WITH_LOADING_FORM);
		}
	}
	...
	public void onConfigTableFail(DataPackage dataPackage) {
		...
		case ProtocolConstants.ErrorCode.TABLE_PASSWORD_TOO_LONG_ERROR:
			GameGlobal.alert.showAlert(this, Alert.OK_TYPE, new String[] {
					"Thiết lập bàn thất bại", "Mật khẩu quá dài" });
			break;
		case ProtocolConstants.ErrorCode.NOT_ENOUGH_MONEY_TO_SET_TABLE_CONFIG_ERROR:
			GameGlobal.alert.showAlert(this, Alert.OK_TYPE, new String[] {
					"Thiết lập bàn thất bại", "Bạn không đủ tiền" });
			break;
		case ProtocolConstants.ErrorCode.NO_AUTHEN_ERROR:
			GameGlobal.alert.showAlert(this, Alert.OK_TYPE,
					"Mất kết nối với máy chủ", 99);
		...
		default:
			GameGlobal.alert.showAlert(this, Alert.OK_TYPE,
					new String[] { "Thiết lập bàn thất bại" });
			break;
		}
	}
	...
	public void onConfigTableChange(DataPackage dataPackage) {
		...
		if (bid != Global.currentTable.getBid()) {
			...
			if (!Global.currentUser.getName().equals(
				...
				if (Global.currentUser.getMoney() < Global.currentTable
						.getBid()) {
					GameGlobal.alert
							.showAlert(this, Alert.OK_TYPE, new String[] {
									"Điểm cược đã thay đổi thành " + bid,
									"Bạn không đủ tiền để chơi ở bàn này" }, 98);
				} else {
					GameGlobal.alert.showAlert(this, Alert.YES_NO_TYPE,
							new String[] {
									"Điểm cược đã thay đổi thành " + bid,
									"Bạn có muốn chơi tiếp?" }, 97);
				}
			}
		}
	}
	...
	public void onJoinRoomFail(DataPackage dataPackage) {
		...
		case ProtocolConstants.ErrorCode.NOT_ENOUGN_MONEY_TO_JOIN_ROOM_ERROR:
			GameGlobal.alert.showAlert(this, Alert.OK_TYPE,
					"Bạn không đủ tiền để vào phòng");
			break;
