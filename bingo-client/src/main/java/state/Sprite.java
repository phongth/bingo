...
/**
 * Class dùng để lưu thông tin xử lý về các đối tượng Image và tự động cập nhật
 * màn hình thông qua manager
 * 
 * @author ptran2
 * @version 0.1
 */
public class Sprite {
	 ...
	 * 
	 * @param image
	 *            - the Image to use for Sprite
	 * @param x
	 *            - Tọa độ x khởi tạo cho Sprite
	 * @param y
	 *            - Tọa độ y khởi tạo cho Sprite
	 */
	 ...
	 *            - Tọa độ y của Sprite
	 * @param width
	 *            - Chiều rộng của Sprite
	 * @param height
	 *            - Chiều dài của Sprite
	 */
	...
	public Sprite setMouseMoveListener(MouseMoveListener mouseMoveListener) {
		if (manager == null) {
			throw new IllegalArgumentException(
					"setMouseMoveListener : manager is null");
		}
	...
	public Sprite setTimerListener(TimerListener timerListener) {
		if (manager == null) {
			throw new IllegalArgumentException(
					"setTimerListener : manager is null");
		}
	...
	/**
	 * Cập nhật tọa độ cho Sprite, vị trí mới của Sprite sẽ được tự động cập
	 * nhật vào Graphic
	 * 
	 * @param x
	 *            - Tọa độ x
	...
	/**
	 * Di chuyển Sprite 1 khoảng dx, dy , vị trí mới của Sprite sẽ được tự động
	 * cập nhật vào Graphic
	 * 
	 * @param dx
	 *            - Khoảng di chuyển x
	 * @param dy
	...
	/**
	 * Wrapper lại hàm paint(g) của Sprite, sử dụng cho class CustomSprite
	 * implement lại
	 * 
	 * @param g
	 *            - Graphic
	...
	public boolean checkCollides(Sprite linkedSprite) {
		...
		return checkCollides(linkedSprite.getRealX(), linkedSprite.getRealY(),
				linkedSprite.getWidth(), linkedSprite.getHeight());
	}

	/**
	 * Kiểm tra va chạm với 1 điểm bằng cách kiểm tra xem điểm tương ứng trên
	 * Sprite có trong suốt hay không, nếu là điểm không trong suốt thì được coi
	 * là va chạm
	 * 
	 * @param x
	...
	public boolean checkCollides(int x, int y) {
		...
		if ((value == 0x00000000) || ((value & 0xFF000000) == 0x00000000)
				|| (value == 0xFFFFFFFF)) {
			return false;
		}
		return true;
	...
	public boolean checkCollides(int x, int y, int width, int height) {
		...
		if ((x + width < getRealX()) || (getRealX() + getWidth() < x)
				|| (y + height < getRealY()) || (getRealY() + getHeight() < y)) {
			return false;
		}
		return true;
	}
