...
public class DataPackage {
	...
	private byte[] data; // 4 bytes đầu là header, 4 byte tiếp theo là độ dài, còn lại là data
	...
	public void putByteArray(byte[] bytes) {
		if (data.length - putDataIndex < bytes.length + 4) {
			throw new ArrayIndexOutOfBoundsException("putByteArray: No more space in data package: " + putDataIndex + " / require space: " + bytes.length);
		}
		putInt(bytes.length);
	...
	public byte[] getAllData() {
		System.arraycopy(SerialUtil.serialNumber(putDataIndex - 8), 0, data, 4, 4); // write the length of data to data package 
