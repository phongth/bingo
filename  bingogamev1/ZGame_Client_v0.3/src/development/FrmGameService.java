...
public class FrmGameService extends GameForm implements EventListener {
	...
	public void draw(Graphics g) {
		...
		for (int i = 0; i < Global.gameServices.size(); i++) {
			...
			GameService gameService = (GameService) Global.gameServices.elementAt(i);
			...
			text8.drawString(g, gameServiceName, color, 26, DY + i * DISTANCE, GameConstants.VCENTER_LEFT_ANCHOR);
		}
	...
	public void onActionPerform(Event event) {
		...
		if (action.equals(MENU[0])) { // Cập nhật danh sách
		  ...
		  GameGlobal.nextState(Global.frmChangePassword, null, Transformer.TRANSFORM_WITH_LOADING_FORM);
		} else if (action.equals(MENU[2])) { // Đăng suất
			GameGlobal.alert.showAlert(this, Alert.YES_NO_TYPE, "Bạn có muốn thoát ra?", 99);
		}
	}

	public void alertEventPerform(int alertType, int eventType, int alertId) {
		if (alertId == 99 && alertType == Alert.YES_NO_TYPE && eventType == Alert.YES_BUTTON) { // Thoát ra
	...
	public void keyReleased(int keyCode) {
		...
		case Key.FIRE:
		case Key.K_5:
			GameService gameService = (GameService) Global.gameServices.elementAt(chooseIndex);
