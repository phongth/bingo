...
public class Transformer extends GameForm implements TimerListener {
  ...
  private Image logoImage;
	...
	public void transform(GameForm currentState, GameForm nextState, final Hashtable params, int transformType) {
		this.currentState = currentState;
		this.nextState = nextState;
		this.transformType = transformType;
		
		if (transformType != TRANSFORM_WITH_LOADING_FORM) {
			nextStateImage = Image.createImage(GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT);
			currentStateImage = Image.createImage(GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT);
		...
		case TRANSFORM_MOVE_LEFT:
			currentStateLocation = Point.createNewOrSetValue(currentStateLocation, 0, 0);
			nextStateLocation = Point.createNewOrSetValue(nextStateLocation, GameConstants.SCREEN_WIDTH, 0);
			break;
		...
		case TRANSFORM_CLIP_WIDE_OPEN_FROM_CENTER:
			clipSize = Point.createNewOrSetValue(clipSize, 0, GameConstants.SCREEN_HEIGHT);
			clipLocation = Point.createNewOrSetValue(clipLocation, GameConstants.SCREEN_WIDTH / 2, 0);
			break;
		case TRANSFORM_CLIP_CLOSE_TO_CENTER:
			clipSize = Point.createNewOrSetValue(clipSize, GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT);
		...
		if (!this.nextState.isRunning && (transformType != TRANSFORM_WITH_LOADING_FORM)) {
  ...
  public void draw(Graphics g) {
      ...
      if (GameConstants.IS_240x320_SCREEN) {
        ...
        text8.drawString(g, "Đang tải...", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, 264, GameConstants.TOP_HCENTER_ANCHOR);
        x = 62;
        y = 247;
      } else {
        g.drawImage(logoImage, GameConstants.SCREEN_WIDTH / 2, 32, GameConstants.TOP_HCENTER_ANCHOR);
        text8.drawString(g, "Đang tải...", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, 193, GameConstants.TOP_HCENTER_ANCHOR);
        x = 101;
        y = 172;
      }
      for (int i = 1; i < 11; i++) {
		...
		case TRANSFORM_MOVE_LEFT:
			if (nextStateLocation.x > 0) {
				...
				g.drawImage(currentStateImage, currentStateLocation.x, currentStateLocation.y, GameConstants.TOP_LEFT_ANCHOR);
				g.drawImage(nextStateImage, nextStateLocation.x, nextStateLocation.y, GameConstants.TOP_LEFT_ANCHOR);
			} else {
		...
		case TRANSFORM_CLIP_WIDE_OPEN_FROM_CENTER:
			if (clipSize.x < GameConstants.SCREEN_WIDTH) {
				...
				g.setClip(clipLocation.x, clipLocation.y, clipSize.x, clipSize.y);
				g.drawImage(nextStateImage, 0, 0, GameConstants.TOP_LEFT_ANCHOR);
			} else {
				g.setClip(0, 0, GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT);
				g.drawImage(nextStateImage, 0, 0, GameConstants.TOP_LEFT_ANCHOR);
		...
		case TRANSFORM_CLIP_BREAK_OLD_FORM:
			if (centerAreaSize < GameConstants.SCREEN_HEIGHT) {
				...
				g.drawImage(currentStateImage, 0, 0, GameConstants.TOP_LEFT_ANCHOR);
				g.setClip((GameConstants.SCREEN_WIDTH + centerAreaSize) / 2, 0, width, height);
				g.drawImage(currentStateImage, 0, 0, GameConstants.TOP_LEFT_ANCHOR);
				g.setClip(0, (GameConstants.SCREEN_HEIGHT + centerAreaSize) / 2, width, height);
				g.drawImage(currentStateImage, 0, 0, GameConstants.TOP_LEFT_ANCHOR);
				g.setClip((GameConstants.SCREEN_WIDTH + centerAreaSize) / 2, (GameConstants.SCREEN_HEIGHT + centerAreaSize) / 2, width, height);
				g.drawImage(currentStateImage, 0, 0, GameConstants.TOP_LEFT_ANCHOR);
				centerAreaSize += MOVE_STEP;
			} else {
				g.setClip(0, 0, GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT);
