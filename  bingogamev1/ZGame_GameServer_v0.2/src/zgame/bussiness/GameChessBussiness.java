...
public class GameChessBussiness extends GameComponent {
	...
	private static final int[][] upSide = new int[][]{{0, 3}, {2, 3}, {4, 3}, {6, 3}, {8, 3}, {1, 2}, {7, 2}, {0, 0}, {8, 0}, {1, 0}, {7, 0}, {2, 0}, {6, 0}, {3, 0}, {5, 0}, {4, 0}};
	private static final int[][] downSide = new int[][]{{0, 6}, {2, 6}, {4, 6}, {6, 6}, {8, 6}, {1, 7}, {7, 7}, {0, 9}, {8, 9}, {1, 9}, {7, 9}, {2, 9}, {6, 9}, {3, 9}, {5, 9}, {4, 9}};
	...
	public void init() {
		...
		// Tìm xem nếu người đánh thắng ván trước đó vẫn còn trong bàn thì cho ng đó đánh trước
		...
		// Nếu người đánh thắng ván trước không còn trong bàn thì cho chủ bàn đánh trước
		if (redSide == null) {
			...
			if (currentAvailableMoves != null) {
				for (int i = 0; i < currentAvailableMoves.length; i++) {
					if (currentAvailableMoves[i] != null) {
						...
						if ((column - chessMans[side][chessIndex].getColumn() == currentAvailableMoves[i][0]) && row - chessMans[side][chessIndex].getRow() == currentAvailableMoves[i][1]) {
							isMoveAlowed = true;
							ChessMan enemy = getChessMan(oppSide, column, row);
							if (enemy != null) { // vị trí vừa đi đến đang có quân cờ của đối thủ
								if (enemy.getType() == ChessMan.VUA_TYPE) {
									isEndGame = true;
								}
								killChessMan(enemy);
							}
							chessMans[side][chessIndex].changePosition(column, row); // Dịch chuyển quân cờ trên bàn cờ đến vị trí client gửi lên phía server
	...
	public void initChessBoard() {
		...
		chessMans[ChessMan.RED_SIDE][0] = new ChessMan(ChessMan.RED_SIDE, ChessMan.TOT_TYPE, redSidePosition[0][0], redSidePosition[0][1]);
		chessMans[ChessMan.RED_SIDE][1] = new ChessMan(ChessMan.RED_SIDE, ChessMan.TOT_TYPE, redSidePosition[1][0], redSidePosition[1][1]);
		chessMans[ChessMan.RED_SIDE][2] = new ChessMan(ChessMan.RED_SIDE, ChessMan.TOT_TYPE, redSidePosition[2][0], redSidePosition[2][1]);
		chessMans[ChessMan.RED_SIDE][3] = new ChessMan(ChessMan.RED_SIDE, ChessMan.TOT_TYPE, redSidePosition[3][0], redSidePosition[3][1]);
		chessMans[ChessMan.RED_SIDE][4] = new ChessMan(ChessMan.RED_SIDE, ChessMan.TOT_TYPE, redSidePosition[4][0], redSidePosition[4][1]);
		chessMans[ChessMan.RED_SIDE][5] = new ChessMan(ChessMan.RED_SIDE, ChessMan.PHAO_TYPE, redSidePosition[5][0], redSidePosition[5][1]);
		chessMans[ChessMan.RED_SIDE][6] = new ChessMan(ChessMan.RED_SIDE, ChessMan.PHAO_TYPE, redSidePosition[6][0], redSidePosition[6][1]);
		chessMans[ChessMan.RED_SIDE][7] = new ChessMan(ChessMan.RED_SIDE, ChessMan.XE_TYPE, redSidePosition[7][0], redSidePosition[7][1]);
		chessMans[ChessMan.RED_SIDE][8] = new ChessMan(ChessMan.RED_SIDE, ChessMan.XE_TYPE, redSidePosition[8][0], redSidePosition[8][1]);
		chessMans[ChessMan.RED_SIDE][9] = new ChessMan(ChessMan.RED_SIDE, ChessMan.MA_TYPE, redSidePosition[9][0], redSidePosition[9][1]);
		chessMans[ChessMan.RED_SIDE][10] = new ChessMan(ChessMan.RED_SIDE, ChessMan.MA_TYPE, redSidePosition[10][0], redSidePosition[10][1]);
		chessMans[ChessMan.RED_SIDE][11] = new ChessMan(ChessMan.RED_SIDE, ChessMan.TUONG_TYPE, redSidePosition[11][0], redSidePosition[11][1]);
		chessMans[ChessMan.RED_SIDE][12] = new ChessMan(ChessMan.RED_SIDE, ChessMan.TUONG_TYPE, redSidePosition[12][0], redSidePosition[12][1]);
		chessMans[ChessMan.RED_SIDE][13] = new ChessMan(ChessMan.RED_SIDE, ChessMan.SY_TYPE, redSidePosition[13][0], redSidePosition[13][1]);
		chessMans[ChessMan.RED_SIDE][14] = new ChessMan(ChessMan.RED_SIDE, ChessMan.SY_TYPE, redSidePosition[14][0], redSidePosition[14][1]);
		chessMans[ChessMan.RED_SIDE][15] = new ChessMan(ChessMan.RED_SIDE, ChessMan.VUA_TYPE, redSidePosition[15][0], redSidePosition[15][1]);

		int[][] blackSidePosition = upSide;
		chessMans[ChessMan.BLACK_SIDE][0] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.TOT_TYPE, blackSidePosition[0][0], blackSidePosition[0][1]);
		chessMans[ChessMan.BLACK_SIDE][1] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.TOT_TYPE, blackSidePosition[1][0], blackSidePosition[1][1]);
		chessMans[ChessMan.BLACK_SIDE][2] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.TOT_TYPE, blackSidePosition[2][0], blackSidePosition[2][1]);
		chessMans[ChessMan.BLACK_SIDE][3] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.TOT_TYPE, blackSidePosition[3][0], blackSidePosition[3][1]);
		chessMans[ChessMan.BLACK_SIDE][4] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.TOT_TYPE, blackSidePosition[4][0], blackSidePosition[4][1]);
		chessMans[ChessMan.BLACK_SIDE][5] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.PHAO_TYPE, blackSidePosition[5][0], blackSidePosition[5][1]);
		chessMans[ChessMan.BLACK_SIDE][6] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.PHAO_TYPE, blackSidePosition[6][0], blackSidePosition[6][1]);
		chessMans[ChessMan.BLACK_SIDE][7] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.XE_TYPE, blackSidePosition[7][0], blackSidePosition[7][1]);
		chessMans[ChessMan.BLACK_SIDE][8] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.XE_TYPE, blackSidePosition[8][0], blackSidePosition[8][1]);
		chessMans[ChessMan.BLACK_SIDE][9] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.MA_TYPE, blackSidePosition[9][0], blackSidePosition[9][1]);
		chessMans[ChessMan.BLACK_SIDE][10] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.MA_TYPE, blackSidePosition[10][0], blackSidePosition[10][1]);
		chessMans[ChessMan.BLACK_SIDE][11] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.TUONG_TYPE, blackSidePosition[11][0], blackSidePosition[11][1]);
		chessMans[ChessMan.BLACK_SIDE][12] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.TUONG_TYPE, blackSidePosition[12][0], blackSidePosition[12][1]);
		chessMans[ChessMan.BLACK_SIDE][13] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.SY_TYPE, blackSidePosition[13][0], blackSidePosition[13][1]);
		chessMans[ChessMan.BLACK_SIDE][14] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.SY_TYPE, blackSidePosition[14][0], blackSidePosition[14][1]);
		chessMans[ChessMan.BLACK_SIDE][15] = new ChessMan(ChessMan.BLACK_SIDE, ChessMan.VUA_TYPE, blackSidePosition[15][0], blackSidePosition[15][1]);
	}
			...
			if (chessMan.getType() == ChessMan.TOT_TYPE) {
				...
				for (int j = 0; j < currentAvailableMoves.length; j++) {
					if (currentAvailableMoves[j] != null) {
						...
						if (opponentKing.getColumn() == nextColumn) {
							ChessMan[] chessMans2 = getAllChessManInColumn(nextColumn);// Lấy các con cờ cùng cột với tướng
				...
				for (int j = 0; j < currentAvailableMoves.length; j++) {
					if (currentAvailableMoves[j] != null) {
						int nextColumn = chessMan.getColumn() + currentAvailableMoves[j][0];
						int nextRow = chessMan.getRow() + currentAvailableMoves[j][1];
						if (getChessMan(side, nextColumn, nextRow) != null || getChessMan(ChessMan.BLACK_SIDE, (nextColumn + chessMan.getColumn()) / 2, (nextRow + chessMan.getRow()) / 2) != null
					...
					if (currentAvailableMoves[j] != null) {
						...
						if (Math.abs(currentAvailableMoves[j][0]) == 2) {
							if ((getChessMan(side, chessMan.getColumn() + (currentAvailableMoves[j][0] > 0 ? 1 : -1), chessMan.getRow()) != null)
									|| getChessMan(getOpponentSide(side), chessMan.getColumn() + (currentAvailableMoves[j][0] > 0 ? 1 : -1), chessMan.getRow()) != null) {
								currentAvailableMoves[j] = null;
								continue;
							}
							...
							if ((getChessMan(side, chessMan.getColumn(), chessMan.getRow() + (currentAvailableMoves[j][1] > 0 ? 1 : -1)) != null)
									|| getChessMan(getOpponentSide(side), chessMan.getColumn(), chessMan.getRow() + (currentAvailableMoves[j][1] > 0 ? 1 : -1)) != null) {
								currentAvailableMoves[j] = null;
								continue;
							}
						}
	...
	private void filterToDontMakeTheKingsFaceToFace(ChessMan chessMan, int[][] currentAvailableMoves) {
		// Nếu là quân Tướng di chuyển thì không cần check nữa vì đã check ở trên rồi
		if (chessMan.getType() == ChessMan.VUA_TYPE) {
			return;
		}
	...
	private int[][] getAvailableMoveByRow(ChessMan chessMan) {
		...
		for (int j = 0; j < chessManInSameRow.length; j++) {
			...
			} else if ((maxColumn != NUMBER_OF_COLUMN) && (afterMaxEnemyColumn == -1) && (chessManInSameRow[j] != null) && (chessManInSameRow[j].getSide() != chessMan.getSide())) {
	...
	private int[][] getAvailableMoveByColumn(ChessMan chessMan) {
		...
		for (int j = 0; j < chessManInSameColumn.length; j++) {
			...
			} else if ((maxRow != NUMBER_OF_ROW) && (afterMaxEnemyRow == -1) && (chessManInSameColumn[j] != null) && (chessManInSameColumn[j].getSide() != chessMan.getSide())) {
				afterMaxEnemyRow = j;
			}
		}
			...
			if (chessMans[opponentSide][i] != null) {
				...
				for (int j = 0; j < availableMoveTmp.length; j++) {
					if (availableMoveTmp[j] != null) {
						if ((availableMoveTmp[j][0] + chessMans[opponentSide][i].getColumn() == king.getColumn()) && (availableMoveTmp[j][1] + chessMans[opponentSide][i].getRow() == king.getRow())) {
							return true;
						}
					}
				}
