...
public class ImageTextForMidp2 extends ImageFontForMidp2 implements ImageText {
	...
	public int drawParagraph(Graphics g, String text, int color, int width, int lineDistance, int x, int y) {
		...
		for (int i = 0; i < textHashCodeCache.length; i++) {
			if (hashCode == textHashCodeCache[i]) {
				for (int j = 0; j < beginIndexCache[i].length; j++) {
					drawSubstring(g, text, color, beginIndexCache[i][j], lenCache[i][j], x, y + j * (getHeight() + lineDistance), Graphics.TOP | Graphics.LEFT);
				}
				return y + beginIndexCache[i].length * (getHeight() + lineDistance);
			}
		}
		...
		int numberOfLine = text.length() / 5; // Dự đoán ít nhất là 5 ký tự trên 1 dòng
		beginIndexCache[index] = new short[numberOfLine];
		lenCache[index] = new short[numberOfLine];
		
		// Nếu chưa có thì tiến hành phân tách đoạn văn thành các dòng, lưu vào cache và vẽ
		short lineIndex = 0;
		short charIndex = 0;
		short beginIndex = 0;
		...
		while (charIndex < text.length()) {
			...
			if(c == '\n') {
				...
				drawSubstring(g, text, color, beginIndexCache[index][lineIndex], lenCache[index][lineIndex], x, y + lineIndex * (getHeight() + lineDistance), Graphics.TOP | Graphics.LEFT);
				beginIndex = (short) (charIndex + 1);
				currentLineLen = 0;
				isNextLine = true;
			} else if(currentLineLen + charLen > width) { // Nếu độ dài của dòng hiện tại quá độ dài cho phép
				beginIndexCache[index][lineIndex] = beginIndex;
				if (lastSpaceIndex > -1) { // Nếu có dấu space trước đó thì ngắt tại vị trí space
			...
			if (isNextLine) {
				// Kiểm tra xem đã hết cache chưa, nếu đã hết thì tạo cache gấp rưỡi và copy dữ liệu sang
				if (lineIndex >= beginIndexCache[index].length) {
					resizecache(index, beginIndexCache[index].length + beginIndexCache[index].length / 2);
				}
				lastSpaceIndex = -1;
				lineIndex++;
			}
