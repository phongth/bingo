...
public class FrmLogin extends GameForm implements TimerListener, EventListener {
    ...
    passwordTextField = new TextField(TextField.CONSTRAINT_PASSWORD, Constants.TEXT_FIELD_STYLE);
		...
		if (Global.loginInfo.isSaveUserNameAndPassword() || Global.loginInfo.isAutoLogin()) {
			...
			// Nếu không phải là tự động đăng nhập thì xác nhận user ngầm để tăng performent
			if (!Global.loginInfo.isAutoLogin() && !Global.loginInfo.getUserName().equals(Global.validedUsername)) {
			  SocketClientUtil.privateLogin();
			}
		}

		MENU = new String[] { "Đký tài khoản", "Gọi điện hỗ trợ", "Trợ giúp", "Thoát" };
    ...
    if (menu.isShowing()) {
      ...
      if (chooseIndex < 2) {
        text8.drawString(g, "Đăng nhập", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, GameConstants.SCREEN_HEIGHT - 5, GameConstants.BOTTOM_HCENTER_ANCHOR);
      }
    }
    text8.drawString(g, "Hotline: 19001530", Color.YELLOW_CODE, GameConstants.SCREEN_WIDTH - 5, GameConstants.SCREEN_HEIGHT - 25, GameConstants.BOTTOM_RIGHT_ANCHOR);
	...
	private void drawFor240x320(Graphics g) {
		...
		text8.drawString(g, "Tự động đăng nhập", Color.WHITE_CODE, 72, 213, GameConstants.TOP_LEFT_ANCHOR);
		text8.drawString(g, "Nhớ mật khẩu", Color.WHITE_CODE, 72, 229, GameConstants.TOP_LEFT_ANCHOR);
	...
	private void drawFor320x240(Graphics g) {
		...
		text8.drawString(g, "Tự động đăng nhập", Color.WHITE_CODE, 90, 180, GameConstants.TOP_LEFT_ANCHOR);
		text8.drawString(g, "Nhớ mật khẩu", Color.WHITE_CODE, 90, 196, GameConstants.TOP_LEFT_ANCHOR);
	...
	public void keyReleased(int keyCode) {
		...
		case Key.ENTER:
			if (chooseIndex < 2) {
				...
				Global.loginInfo.setSaveUserNameAndPassword(!Global.loginInfo.isSaveUserNameAndPassword());
			}
			break;
		case Key.UP:
			chooseIndex--;
	...
	public void onActionPerform(Event event) {
	  //	  "Đký tài khoản", "Gọi điện hỗ trợ", "Trợ giúp", "Thoát"
		...
		if (action.equals(MENU[0])) { // Đký tài khoản
		  GameGlobal.nextState(Global.frmRegister, null, Transformer.TRANSFORM_WITH_LOADING_FORM);
