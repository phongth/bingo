...
public class ChessMan extends Sprite {
	...
	public ChessMan(int side, int type, Image image, int column, int row) {
		super(image, FrmChess.BOARD_DX + FrmChess.CELL_SIZE * column, FrmChess.BOARD_DY +  FrmChess.CELL_SIZE * row, GameConstants.CENTER_ANCHOR);
		this.type = type;
		this.column = column;
		this.row = row;
		this.side = side;
		...
		case TOT_TYPE:
			if (row > 5) {
				availableMove = new int[][] {{0, -1}, null, null}; // Để trống 2 phần tử cuối dành cho ăn ngang khi sang sông
			} else {
				availableMove = new int[][] {{0, 1}, null, null}; // Để trống 2 phần tử cuối dành cho ăn ngang khi sang sông
			}
			break;
	...
	public Sprite move(int dColumn, int dRow) {
		...
		setPosition(FrmChess.BOARD_DX + FrmChess.CELL_SIZE * column, FrmChess.BOARD_DY + FrmChess.CELL_SIZE * row);
		return this;
	}
