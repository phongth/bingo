...
public class FriendBussiness implements AlertListener {
  ...
  public void onAddFriendNotify(DataPackage dataPackage) {
    ...
    GameGlobal.alert.showAlert(this, Alert.YES_NO_TYPE, new String[] {addFriendFromUser + " đề nghị được kết bạn.", "Bạn có đồng ý không?"}, 99);
  }
  
  public void onAddFriendSuccess(DataPackage dataPackage) {
    String toUser = dataPackage.nextString();
    GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Kết bạn với " + toUser + " thành công");
    SocketClientUtil.requestFriendList();
  }
  
  public void onAddFriendFailUserDeny(DataPackage dataPackage) {
    String toUser = dataPackage.nextString();
    GameGlobal.alert.showAlert(this, Alert.OK_TYPE, new String[]{"Kết bạn thất bại.", "Người chơi " + toUser, " từ chối kết bạn."});
  }
  
  public void onAddFriendFailUserNotExist(DataPackage dataPackage) {
    String toUser = dataPackage.nextString();
    GameGlobal.alert.showAlert(this, Alert.OK_TYPE, new String[]{"Kết bạn thất bại.", "Người chơi " + toUser, " không tồn tại."});
  }
  ...
  public void onGetFriendList(DataPackage dataPackage) {
    ...
    for (int i = 0; i < numberOfFriend; i++) {
      ...
      for (int j = 0; j < Constants.GAME_ID.length; j++) {
        ...
        if (index > 0) {
          locationInfo = locationInfo.substring(0, index) + Constants.GAME_NAME[j];
          break;
        }
      }
  ...
  public void alertEventPerform(int alertType, int eventType, int alertId) {
    if (alertId == 99) { // Alert from onAddFriendNotify : {fromUser + " đề nghị được kết bạn.", "Bạn có đồng ý không?"}
      
      // If agree
      if (eventType == Alert.YES_BUTTON) {
        DataPackage agreeRequestDataPackage = new DataPackage(ProtocolConstants.RequestHeader.ADD_FRIEND_AGREE_REQUEST);
      ...
      if (eventType == Alert.NO_BUTTON) {
        DataPackage denyRequestDataPackage = new DataPackage(ProtocolConstants.RequestHeader.ADD_FRIEND_DENY_REQUEST);
