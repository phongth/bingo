...
public class AuthenticateBussiness {
	...
	public static void registerUser(Server server, DataPackage inputDataPackage)
			throws SQLException {
		...
		if (errorCode < 0) {
			DataPackage dataPackage = new DataPackage(
					ProtocolConstants.ResponseHeader.REGISTER_FAIL_RESPONSE);
		...
		try {
			int defaultMoney = 5000; // TODO: đang cho mỗi user khi tạo tài
										// khoản là 5000
			authenticateDao.createUser(username, md5Pass, defaultMoney,
					providerId);
		...
		DataPackage dataPackage = new DataPackage(
				ProtocolConstants.ResponseHeader.REGISTER_SUCCESS_RESPONSE);
		dataPackage.putString(username);
		server.write(dataPackage);
	...
	public static void changePassword(Server server,
			DataPackage inputDataPackage) throws SQLException {
		AuthenticateDao authenticateDao = AuthenticateDao.createInstance();
		...
		if (user == null) {
			DataPackage dataPackage = new DataPackage(
					ProtocolConstants.ResponseHeader.CHANGR_PASSWORD_FAIL_RESPONSE);
			dataPackage.putString(username);
			dataPackage
					.putInt(ProtocolConstants.ErrorCode.USER_NOT_EXIST_ERROR);
			server.write(dataPackage);
			return;
		}
		...
		if (!user.getMd5Pass().equals(oldPassMd5)) {
			...
			dataPackage.putString(username);
			dataPackage
					.putInt(ProtocolConstants.ErrorCode.PASSWORD_NOT_CORRECT_ERROR);
			server.write(dataPackage);
			return;
		}
	...
	public static void checkAuthenticate(Server server,
			DataPackage inputDataPackage) throws SQLException {
		int userId = -1;
		String username = inputDataPackage.nextString();
		String encodedData = inputDataPackage.nextString();
		...
		if (userId > -1) {
			...
			DataPackage dataPackage = new DataPackage(
					ProtocolConstants.ResponseHeader.AUTHENTICATE_SUCCESS_RESPONSE);
			dataPackage.putString(username);
			...
			Map<String, GameService> gameServices = Main.gameServiceController
					.getGameServices();
			...
			DataPackage dataPackage = new DataPackage(
					ProtocolConstants.ResponseHeader.AUTHENTICATE_FAIL_RESPONSE);
			dataPackage.putString(username);
	...
	private static int checkLoginByPassword(String username, String encodedData)
			throws SQLException {
		AuthenticateDao authenticateDao = AuthenticateDao.createInstance();
		if (!isExist(username, authenticateDao)) {
			return -1;
		...
		if (salt == null) {
			log.warn("User " + username
					+ " authen with no salt cached in server.");
			return -1;
		}

		// Nếu password sai thì trả ra là đăng nhập không thành công
		String reencodedData = MD5
				.toBase64((username + user.getMd5Pass() + salt).getBytes());
	...
	public static void gameServiceListRequest(Server server,
			DataPackage inputDataPackage) {
		DataPackage dataPackage = new DataPackage(
				ProtocolConstants.ResponseHeader.GAME_SERVICE_LIST_RESPONSE);
	...
	public static void requestSalt(Server server, DataPackage inputDataPackage) {
		...
		// Gửi salt cho client
		DataPackage dataPackage = new DataPackage(
				ProtocolConstants.ResponseHeader.SALT_REPONSE);
	...
	public static String getUsernameByUserId(int userId) throws SQLException {
		...
		String username = Global.userNameByUserIdMap
	...
	protected static User getUserInfo(String username,
			AuthenticateDao authenticateDao) throws SQLException {
	...
	protected static void updateCache(String username,
			AuthenticateDao authenticateDao) throws SQLException {
		User user = authenticateDao.getUserInfo(username);
		if (user != null) {
			Global.userInfoCache.put(username, user);
			Global.userNameByUserIdMap.put(String.valueOf(user.getUserId()),
					username);
		}
	}

	protected static void updateCache(int userId,
			AuthenticateDao authenticateDao) throws SQLException {
