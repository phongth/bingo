...
public class FriendBussiness {
  ...
  public static void onAddFriendRequest(Server server, DataPackage dataPackage) throws SQLException {
    ...
    if (!AuthenticateBussiness.isExist(toUser, authenticateDao)) {
      DataPackage userNotExistDataPackage = new DataPackage(ProtocolConstants.ResponseHeader.ADD_FRIEND_FAIL_USER_NOT_EXIST_RESPONSE);
  ...
  public static void onAddFriendAgreeRequest(Server server, DataPackage dataPackage) throws SQLException {
    ...
    if (!isAlreadyAddFriend) {
      log.warn("FriendBussiness: onAddFriendAgreeRequest: User did NOT request friend (fromUser: " + fromUser + ", toUser: " + toUser + ")");
      return;
    }
