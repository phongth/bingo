...
public class Initialize {
  private static Logger log = Logger.getLogger(Initialize.class);
    ...
    log.info("Init log4j done");
		...
		String gameIds = get("GAME_ID").trim(); // GAME_ID = tala : tlmb : cotuong : caro
		List<String> gameIdList = getElements(gameIds, ':');
		String gameNames = get("GAME_NAME").trim(); // GAME_NAME = Tá lả : TLMB : Cờ tướng : Caro
		...
		if (defaultNumberOfRoom == null) {
			throw new IllegalArgumentException("ERROR: Initialize : init : Property DEFAULT_NUMBER_OF_ROOM is not exist. Plz check your config file.");
		}
		
		int[] defaultNumberOfTablePerRoom = getElementsToInt("DEFAULT_NUMER_OF_TABLE_PER_ROOM", ':');
		if (defaultNumberOfTablePerRoom == null) {
			throw new IllegalArgumentException("ERROR: Initialize : init : Property DEFAULT_NUMER_OF_TABLE_PER_ROOM is not exist. Plz check your config file.");
		}
		
		int[] defaultMaxUserOfRoom = getElementsToInt("DEFAULT_MAX_USER_OF_ROOM", ':');
		if (defaultMaxUserOfRoom == null) {
			throw new IllegalArgumentException("ERROR: Initialize : init : Property DEFAULT_MAX_USER_OF_ROOM is not exist. Plz check your config file.");
		}
		...
		if (maxUserPerTableByGame == null) {
			throw new IllegalArgumentException("ERROR: Initialize : init : Property NUMBER_OF_USER_PER_TABLE_BY_GAME is not exist. Plz check your config file.");
		}
		
		// Lấy min bid theo room type
		int[] minBidByRoomType = getElementsToInt("MIN_BID_BY_ROOM_TYPE", ':');
		if (minBidByRoomType == null) {
			throw new IllegalArgumentException("ERROR: Initialize : init : Property MIN_BID_BY_ROOM_TYPE is not exist. Plz check your config file.");
		}
		...
		if (maxBidByRoomType == null) {
			throw new IllegalArgumentException("ERROR: Initialize : init : Property MAX_BID_BY_ROOM_TYPE is not exist. Plz check your config file.");
		}
	...
	private static void createRoomsAndTable(int[] numberOfRoom, Game game, int[] numberOfTablePerRoom, int maxUserOfTable, int[] maxUserOfRoom) {
		int roomId = 1;
	...
	private static String get(String id) throws IllegalArgumentException {
		...
		if (value == null) {
			throw new IllegalArgumentException("ERROR: Initialize : get : Property " + id + " is not exist. Plz check your config file.");
		}
		return value;
	}
