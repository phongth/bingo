...
public class FrmFriend extends GameForm implements EventListener {
	...
	public void init(Hashtable parameters) {
		...
		MENU = new String[] { "Cập nhật danh sách", "Mời chơi", "Mời chat",
				"Block", "Xóa", "Kết bạn" };
		menu = new Menu(MENU, this, Constants.MENU_STYLE, 0,
				GameConstants.SCREEN_HEIGHT - 45);

		addFriendTextField = new TextField(TextField.CONSTRAINT_USER_NAME,
				Constants.TEXT_FIELD_STYLE);
		addFriendTextField.setWidth(150);
		addFriendTextField.setPosition((GameConstants.SCREEN_WIDTH - 150) / 2,
				GameConstants.SCREEN_HEIGHT / 2);
		addFriendTextField.setClearCharKey(Key.SOFT_RIGHT);

		addFriendLabel = new Sprite(0, GameConstants.SCREEN_HEIGHT / 2 - 30,
				GameConstants.SCREEN_WIDTH, 20)
				.setDrawListener(new DrawListener() {
					public void paint(Sprite source, Graphics g) {
						text8.drawString(g, "Kết bạn với người chơi",
								Color.WHITE_CODE,
								GameConstants.SCREEN_WIDTH / 2, source.getY(),
	...
	public void draw(Graphics g) {
		...
		for (int i = 0; i < Global.friends.size(); i++) {
			...
			if (friend.isOnline()) {
				g.drawImage(onlineImage, 15, DY + i * DISTANCE,
						GameConstants.CENTER_ANCHOR);
			} else {
				g.drawImage(offlineImage, 15, DY + i * DISTANCE,
						GameConstants.CENTER_ANCHOR);
			}
			text8.drawString(g, friend.getUsername(), color, 26, DY + i
					* DISTANCE, GameConstants.VCENTER_LEFT_ANCHOR);
			text8.drawString(g, friend.getLocationInfo(), color,
					GameConstants.SCREEN_WIDTH / 2 - 30, DY + i * DISTANCE,
	...
	public void onActionPerform(Event event) {
		// "Cập nhật danh sách", "Mời chơi", "Mời chat", "Block", "Xóa",
		// "Kết bạn"
		...
		if (action.equals(MENU[0])) { // Cập nhật danh sách
			...
			GameGlobal.alert.showCustomUIAlert(this, new Sprite[] {
					addFriendLabel, addFriendTextField }, new String[] {
					"Thêm bạn", "Hủy" }, Alert.BUTTON_WIDTH_SMALL, 99,
					Alert.HORIZONTAL_ALINE);
		}
	}
