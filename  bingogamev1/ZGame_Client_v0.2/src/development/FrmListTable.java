...
public class FrmListTable extends GameForm implements TimerListener, EventListener {
	...
	public void init(Hashtable parameters) {
		...
		personFocusImages[1] = Image.createImage(personFocusImages[0], 0, 0, personFocusImages[0].getWidth(), personFocusImages[0].getHeight(), Sprite.TRANS_ROT90);
		personFocusImages[2] = Image.createImage(personFocusImages[0], 0, 0, personFocusImages[0].getWidth(), personFocusImages[0].getHeight(), Sprite.TRANS_ROT180);
		personFocusImages[3] = Image.createImage(personFocusImages[0], 0, 0, personFocusImages[0].getWidth(), personFocusImages[0].getHeight(), Sprite.TRANS_ROT270);

		personImages = new Image[4];
		personImages[0] = ImageUtil.getImage("Nguoi.png");
		personImages[1] = Image.createImage(personImages[0], 0, 0, personImages[0].getWidth(), personImages[0].getHeight(), Sprite.TRANS_ROT90);
		personImages[2] = Image.createImage(personImages[0], 0, 0, personImages[0].getWidth(), personImages[0].getHeight(), Sprite.TRANS_ROT180);
		personImages[3] = Image.createImage(personImages[0], 0, 0, personImages[0].getWidth(), personImages[0].getHeight(), Sprite.TRANS_ROT270);
	...
	public void draw(Graphics g) {
		...
		g.drawImage(titleImage, GameConstants.SCREEN_WIDTH / 2, 6, GameConstants.TOP_HCENTER_ANCHOR);
		...
		text8.drawString(g, Global.currentGame.getName() + " - " + levelText + " - Phòng " + Global.currentRoom.getName(), Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, 21, GameConstants.BOTTOM_HCENTER_ANCHOR);

		g.setClip(0, 36, GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT - 61);
		g.setColor(0xA53A06);
		g.fillRect(4, 28, GameConstants.SCREEN_WIDTH - 7, GameConstants.SCREEN_HEIGHT - 55);
		...
		if (Global.currentRoom.numberOfChilds() == 0) {
			...
			text8.drawString(g, "Đang cập nhật danh sách bàn", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, GameConstants.SCREEN_HEIGHT / 2, GameConstants.CENTER_ANCHOR);
			return;
		}

		int currentRow = currentTableIndex / NUMBER_OF_TABLE_PER_ROW;
		int beginTableIndex = (currentRow - 2) * NUMBER_OF_TABLE_PER_ROW; // bàn đầu tiên cần vẽ (bàn đầu tiên của 2 dòng trước đó)
		beginTableIndex = (beginTableIndex < 0) ? 0 : beginTableIndex;
		int endTableIndex = (currentRow + 2) * NUMBER_OF_TABLE_PER_ROW + NUMBER_OF_TABLE_PER_ROW - 1; // bàn cuối cùng cần vẽ
		endTableIndex = (endTableIndex > Global.currentRoom.numberOfChilds() - 1) ? Global.currentRoom.numberOfChilds() - 1 : endTableIndex;
		...
		for (int i = beginTableIndex; i <= endTableIndex; i++) {
			...
			if (i == currentTableIndex) {
				...
				if (table.isLocked()) {
					g.drawImage(lockFocusImage, x + 25, y - 25, GameConstants.CENTER_ANCHOR);
				}
				drawMoneyByCenterAnchor(g, table.getBid(), x, y + 45);
			} else {
				g.drawImage(tableImage, x, y, GameConstants.CENTER_ANCHOR);
				if (table.isLocked()) {
					g.drawImage(lockImage, x + 16, y - 16, GameConstants.CENTER_ANCHOR);
				}
				drawMoneyByCenterAnchor(g, table.getBid(), x, y + 33);
			}
			text8.drawString(g, table.getName().substring(table.getName().length() - 2), Color.WHITE_CODE, x, y, GameConstants.CENTER_ANCHOR);

			for (int j = 0; j < table.getConcurrentUser(); j++) {
				switch (j) {
				case 0:
					if (i == currentTableIndex) {
						g.drawImage(personFocusImages[1], x, y - 28, GameConstants.TOP_HCENTER_ANCHOR);
					} else {
						g.drawImage(personImages[1], x, y - 20, GameConstants.TOP_HCENTER_ANCHOR);
					}
					break;
				case 1:
					if (i == currentTableIndex) {
						g.drawImage(personFocusImages[3], x, y + 32, GameConstants.BOTTOM_HCENTER_ANCHOR);
					} else {
						g.drawImage(personImages[3], x, y + 22, GameConstants.BOTTOM_HCENTER_ANCHOR);
					}
					break;
				case 2:
					if (i == currentTableIndex) {
						g.drawImage(personFocusImages[0], x - 32, y, GameConstants.VCENTER_LEFT_ANCHOR);
					} else {
						g.drawImage(personImages[0], x - 22, y, GameConstants.VCENTER_LEFT_ANCHOR);
					}
					break;
				case 3:
					if (i == currentTableIndex) {
						g.drawImage(personFocusImages[2], x + 32, y, GameConstants.VCENTER_RIGHT_ANCHOR);
					} else {
						g.drawImage(personImages[2], x + 22, y, GameConstants.VCENTER_RIGHT_ANCHOR);
					}
					break;
				}
			}
		...
		if (isShowInputPasswordBox) {
			g.drawImage(popupImage, GameConstants.SCREEN_WIDTH / 2, GameConstants.SCREEN_HEIGHT / 2, GameConstants.CENTER_ANCHOR);
			text8.drawString(g, "Nhập mật khẩu của phòng", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, GameConstants.SCREEN_HEIGHT / 2 - 40, GameConstants.TOP_HCENTER_ANCHOR);
			g.setColor(Color.WHITE_CODE);
			g.fillRect(GameConstants.SCREEN_WIDTH / 2 - 50, GameConstants.SCREEN_HEIGHT / 2 - 10, 100, 20);
			text8.drawString(g, tablePassword.toString(), Color.BLACK_CODE, GameConstants.SCREEN_WIDTH / 2 - 45, GameConstants.SCREEN_HEIGHT / 2 - 8, GameConstants.TOP_LEFT_ANCHOR);
			g.setColor(Color.BLACK_CODE);
			g.drawLine(GameConstants.SCREEN_WIDTH / 2 - 45 + text8.stringWidth(tablePassword.toString()), GameConstants.SCREEN_HEIGHT / 2 - 8, GameConstants.SCREEN_WIDTH / 2 - 45 + text8.stringWidth(tablePassword.toString()), GameConstants.SCREEN_HEIGHT / 2 + 7);
			if (isButtonLeft) {
				g.drawImage(buttonFocusImage, GameConstants.SCREEN_WIDTH / 2 - 78, GameConstants.SCREEN_HEIGHT / 2 + 30, GameConstants.TOP_LEFT_ANCHOR);
				g.drawImage(buttonImage, GameConstants.SCREEN_WIDTH / 2 + 22, GameConstants.SCREEN_HEIGHT / 2 + 30, GameConstants.TOP_LEFT_ANCHOR);
			} else {
				g.drawImage(buttonImage, GameConstants.SCREEN_WIDTH / 2 - 78, GameConstants.SCREEN_HEIGHT / 2 + 30, GameConstants.TOP_LEFT_ANCHOR);
				g.drawImage(buttonFocusImage, GameConstants.SCREEN_WIDTH / 2 + 22, GameConstants.SCREEN_HEIGHT / 2 + 30, GameConstants.TOP_LEFT_ANCHOR);
			}

			text8.drawString(g, "Đồng ý", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2 - 67, GameConstants.SCREEN_HEIGHT / 2 + 33, GameConstants.TOP_LEFT_ANCHOR);
			text8.drawString(g, "Hủy", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2 + 42, GameConstants.SCREEN_HEIGHT / 2 + 33, GameConstants.TOP_LEFT_ANCHOR);
		}
		...
		if ((menu != null) && menu.isShowing()) {
			...
			text8.drawString(g, "Menu", Color.WHITE_CODE, 8, GameConstants.SCREEN_HEIGHT - 5, GameConstants.BOTTOM_LEFT_ANCHOR);
		}
		if (isShowInputPasswordBox) {
			if (tablePassword != null && tablePassword.length() > 0) {
				text8.drawString(g, "Xóa", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH - 8, GameConstants.SCREEN_HEIGHT - 5, GameConstants.BOTTOM_RIGHT_ANCHOR);
			} else {
				text8.drawString(g, "Trở về", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH - 8, GameConstants.SCREEN_HEIGHT - 5, GameConstants.BOTTOM_RIGHT_ANCHOR);
			}
		} else {
			text8.drawString(g, "Trở về", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH - 8, GameConstants.SCREEN_HEIGHT - 5, GameConstants.BOTTOM_RIGHT_ANCHOR);
		}
	...
	private void drawMoneyByCenterAnchor(Graphics g, int money, int x, int y) {
		...
		for (int i = 0; i < moneyStr.length(); i++) {
			images[i] = numberImages[Integer.parseInt(String.valueOf(moneyStr.charAt(i)))];
			totalWidth += images[i].getWidth() + MONEY_CHAR_DISTANCE;
		}
		images[images.length - 1] = moneyImage;
		totalWidth += images[images.length - 1].getWidth() + MONEY_CHAR_DISTANCE;
		x -= totalWidth / 2;
	...
	public void keyReleased(int keyCode) {
		...
		case Key.K_8:
			...
			if (currentTableIndex + NUMBER_OF_TABLE_PER_ROW < Global.currentRoom.numberOfChilds()) {
				currentTableIndex += NUMBER_OF_TABLE_PER_ROW;
				isLockKey = true;
			}
			break;
		...
		case Key.FIRE:
			...
			if (Global.currentTable.getConcurrentUser() >= Global.currentTable.getMaxUser()) {
				GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Bàn đã đầy");
				return;
			}

			if (Global.currentUser.getMoney() < Global.currentTable.getBid()) {
			  GameGlobal.alert.showCustomButtonAlert(this, new String[]{"Nạp điểm", "Quay lại"}, Alert.BUTTON_WIDTH_LARGE, new String[]{"Bạn không đủ điểm để vào bàn", "Để tham gia bạn phải nạp thêm điểm!"}, 99, Alert.VERTICAL_ALINE);
				return;
			}

			if (Global.currentTable.isPlaying()) {
			  GameGlobal.alert.showAlert(this, Alert.OK_TYPE, new String[]{"Phòng đang chơi"});
				return;
			}
			...
			if (isShowInputPasswordBox) {
				if (!isButtonLeft) {
					...
					SocketClientUtil.joinTableRequest(Global.currentTable.getId(), tablePassword.toString());
					isLockKey = true;
					isShowInputPasswordBox = false;
				}
				return;
	...
	private void runAutoCamera() {
		...
		if (currentRow == 0) {
			desY = TABLE_DY;
		} else if (currentRow == Global.currentRoom.numberOfChilds() / NUMBER_OF_TABLE_PER_ROW) {
			if (GameConstants.IS_240x320_SCREEN) {
	...
	public void alertEventPerform(int alertType, int eventType, int alertId) {
		...
		if (alertId == 99 && eventType == 0) {
		  GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Chức năng nạp điểm đang phát triển");
		}
	}
