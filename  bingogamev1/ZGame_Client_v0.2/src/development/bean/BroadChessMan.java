...
public class BroadChessMan extends Sprite {
	  ...
	  // DÃ¹ng sá»‘ 99 Ä‘á»ƒ mÃ´ táº£ viá»‡c cÃ³ thá»ƒ di chuyá»ƒn Ä‘Æ°á»£c cáº£ hÃ ng hay cá»™t
	  ...
	  public BroadChessMan(int side, int type, Image image, int column, int row) {
	    super(image, FrmBroadChess.BOARD_DX + FrmBroadChess.CELL_SIZE * column, FrmBroadChess.BOARD_DY +  FrmBroadChess.CELL_SIZE * row, GameConstants.CENTER_ANCHOR);
	    this.type = type;
	    this.column = column;
	    this.row = row;
	    this.side = side;
	    ...
	    case TOT_TYPE:
	      if (side == RED_SIDE) {
	        availableMove = new int[][] {{0, -1}, {0, -2}, null, null}; // DÃ nh 2 chá»— trá»‘ng cho viá»‡c Äƒn chÃ©o
	      } else {
	        availableMove = new int[][] {{0, 1}, {0, 2}, null, null}; // DÃ nh 2 chá»— trá»‘ng cho viá»‡c Äƒn chÃ©o
	      }
	      break;
	  ...
	  private void checkPassRiver() {
	    // Sau nÆ°á»›c Ä‘i Ä‘áº§u tiÃªn thÃ¬ bá»� nÆ°á»›c nháº£y Ä‘Ã´i cá»§a tá»‘t
	    ...
	    if (side == RED_SIDE) {
	      ...
	      if (row == 7) {
	        ...
	        availableMove = new int[][] {{INFINITE, 0}, {INFINITE, INFINITE}}; 
	        try{
	          image = Image.createImage(GameConstants.IMAGE_FOLDER + "/CoVua15.png" );
	        }catch (IOException e) {
	        }
	      }
	    }
