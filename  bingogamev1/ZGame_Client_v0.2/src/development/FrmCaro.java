...
public class FrmCaro extends GameForm implements TimerListener, DataReceiveListener {
	...
	private static final int MOVE_ERROR_RESPONSE = 2001; // Báo lại cho ng chơi vừa đánh là nước đánh không được chấp nhận
	private static final int MOVE_NOTIFY_RESPONSE = 2002; // Báo nước đi của người vừa đánh
	private static final int WIN_GAME_NOTIFY_RESPONSE = 2005; // Báo cho các user về user đã thắng và danh sách các nước đi thắng
	private static final int INIT_DATA_RESPONSE = 2007; // Thiết đặt các thông số từ server
	...
	public void onRecieveData(DataPackage dataPackage) {
		...
		case MOVE_ERROR_RESPONSE:
			...
			if (error == NOT_YOUR_TURN_ERROR) {
			  ...
			  GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Nước đi không hợp lệ");
			} else {
	...
	public void draw(Graphics g) {
		...
		for (int i = 0; i <= NUMBER_ROW_DISPLAY; i++) {
			...
			if (i < NUMBER_ROW_DISPLAY) {
				text8.drawString(g, String.valueOf(dRow + i), Color.BLACK_CODE, BOARD_DX - 13, BOARD_DY + CELL_SIZE * i, GameConstants.TOP_LEFT_ANCHOR);
			}
		}
		for (int j = 0; j <= NUMBER_COLUMN_DISPLAY; j++) {
			...
			if (j < NUMBER_COLUMN_DISPLAY) {
				text8.drawString(g, String.valueOf(dColumn + j), Color.BLACK_CODE, BOARD_DX + CELL_SIZE * j + 3, BOARD_DY - 14, GameConstants.TOP_LEFT_ANCHOR);
			}
		}
		...
		for (int i = 0; i < NUMBER_COLUMN_DISPLAY; i++) {
			for (int j = 0; j < NUMBER_ROW_DISPLAY; j++) {
				if (matrix[dColumn + i][dRow + j] == CROSS_SIDE) {
					g.drawImage(crossImage, BOARD_DX + i * CELL_SIZE + 2, BOARD_DY + j * CELL_SIZE + 2, GameConstants.TOP_LEFT_ANCHOR);
				} else if (matrix[dColumn + i][dRow + j] == ROUND_SIDE) {
					g.drawImage(roundImage, BOARD_DX + i * CELL_SIZE + 2, BOARD_DY + j * CELL_SIZE + 2, GameConstants.TOP_LEFT_ANCHOR);
				}
			}
		}
		...
		if (currentTurn == mySide) {
			g.drawImage(lastMoveImage, BOARD_DX + 8 + CELL_SIZE * (desX - dColumn), BOARD_DY + 8 + CELL_SIZE * (desY - dRow), GameConstants.CENTER_ANCHOR);
		}

		// Vẽ con trỏ
		g.drawImage(pointerImage, BOARD_DX + 5 + CELL_SIZE * (currentX - dColumn), BOARD_DY + 5 + CELL_SIZE * (currentY - dRow), GameConstants.TOP_LEFT_ANCHOR);
		...
		text8.drawString(g, "" + oponentName, Color.WHITE_CODE, 5, 8, GameConstants.TOP_LEFT_ANCHOR);
		
		
		// Vẽ đếm thời gian và báo lượt đi
		if (currentTurn == mySide) {
			text8.drawString(g, "Lượt đi của " + Global.currentUser.getName(), Color.YELLOW_CODE, GameConstants.SCREEN_WIDTH - 55, 8, GameConstants.TOP_RIGHT_ANCHOR);
		} else {
			text8.drawString(g, "Lượt đi của " + oponentName, Color.YELLOW_CODE, GameConstants.SCREEN_WIDTH - 55, 8, GameConstants.TOP_RIGHT_ANCHOR);
		}
		text8.drawString(g, "00:" + String.valueOf(timeMove), Color.WHITE_CODE, GameConstants.SCREEN_WIDTH - 50, 8, GameConstants.TOP_LEFT_ANCHOR);

		if (Global.userAvatar != null) {
			g.drawImage(Global.userAvatar, 3, GameConstants.SCREEN_HEIGHT - 40, GameConstants.BOTTOM_LEFT_ANCHOR);
		} else {
			g.drawImage(avatarImage, 3, GameConstants.SCREEN_HEIGHT - 40, GameConstants.BOTTOM_LEFT_ANCHOR);
		}
		text8.drawString(g, Global.currentUser.getName(), Color.WHITE_CODE, 5, GameConstants.SCREEN_HEIGHT - 30, GameConstants.TOP_LEFT_ANCHOR);
		...
		case SHOW_WIN_OR_LOST_ALERT_STATUS:
			...
			if (winReason == NORMAL_WIN_REASON) {
				...
				g.drawLine((winPointX1 - dColumn) * 15 + 7 + BOARD_DX, (winPointY1 - dRow) * 15 + 7 + BOARD_DY, (winPointX2 - dColumn) * 15 + 7 + BOARD_DX, (winPointY2 - dRow) * 15 + 7 + BOARD_DY);
			}
...
// text8.drawString(g, currentString, 0xffffff, 58, 225, GameConstants.TOP_LEFT_ANCHOR);
...
// text8.drawString(g, oppMessageChat, Color.WHITE_CODE, 111, 5, GameConstants.TOP_LEFT_ANCHOR);
// }
// }
...
// text8.drawString(g, oppMessageChat, Color.WHITE_CODE, 12, 38, GameConstants.TOP_LEFT_ANCHOR);
// }
// }
