...
public class AuthenticateBussiness implements AlertListener {
  ...
  public void onAuthenticateFail(DataPackage dataPackage) {
    ...
    if (!Global.isAutoLoginDone) {
      ...
      if (isReadyToGoToLoginForm()) {
        ...
        GameGlobal.alert.showAlert(this, Alert.OK_TYPE, new String[]{"Đăng nhập thất bại", errorMessage});
    }
  ...
  public void onGameServiceList(DataPackage dataPackage) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      for (int j = 0; j < Global.gameServices.size(); j++) {
        GameService gameService = (GameService) Global.gameServices.elementAt(j);
  ...
  public void onUpdateClienResponse(DataPackage dataPackage) {
    ...
    GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Chương trình tự động cập nhật phiên bản", 98);
  }
  ...
  public void onReceiveResource(DataPackage dataPackage) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      GameGlobal.imageLocationTable.put(fileName, new Integer(ResourceRms.saveImageData(data)));
      GameGlobal.downloadedCount++;
      Global.frmLogo.message = "Đang tải dữ liệu...";
      Global.frmLogo.message1 = "Đang tải " + GameGlobal.downloadedCount + " / " + GameGlobal.totalDownloadResource;
    }
  }
  ...
  public void onRegisterFail(DataPackage dataPackage) {
    ...
    case ProtocolConstants.ErrorCode.USER_NAME_EXIST_ERROR:
      GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Tên tài khoản đã được sử dụng");
      break;
    ...
    case ProtocolConstants.ErrorCode.USER_NAME_NOT_VALID:
      GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Tên tài khoản không hợp lệ");
      break;
    default:
      GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Tạo tài khoản thất bại");
      break;
    }
  }
  
  public void onChangePasswordSuccess(DataPackage dataPackage) {
    GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Đổi mật khẩu thành công");
  }
  ...
  public void onChangePasswordFail(DataPackage dataPackage) {
    ...
    case ProtocolConstants.ErrorCode.PASSWORD_NOT_CORRECT_ERROR:
      GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Mật khẩu cũ không đúng");
      break;
    case ProtocolConstants.ErrorCode.USER_NOT_EXIST_ERROR:
      GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Tài khoản không tồn tại");
      break;
    default:
      GameGlobal.alert.showAlert(this, Alert.OK_TYPE, "Đổi mật khẩu thất bại");
      break;
    }
  }
}
