...
public class Rms {
	...
	public static Provider loadProvider() {
		...
		try {
			...
			if (recordStore.getNumRecords() > 0) {
				DataPackage dataPackage = new DataPackage(recordStore.getRecord(1));
	...
	public static LoginInfo loadLoginInfo() {
		...
		try {
			...
			if (masterRecordStore.getNumRecords() > 0) {
				loginInfo.setSaveUserNameAndPassword(masterRecordStore.getRecord(1)[0] == 1 ? true : false);
				loginInfo.setAutoLogin(masterRecordStore.getRecord(2)[0] == 1 ? true : false);
				if ((loginInfo.isSaveUserNameAndPassword() || loginInfo.isAutoLogin()) && masterRecordStore.getNumRecords() > 2) {
					loginInfo.setUserName(new String(masterRecordStore.getRecord(3)));
					loginInfo.setPassword(new String(masterRecordStore.getRecord(4)));
					Global.currentUser.setName(loginInfo.getUserName());
					Global.currentUser.setPasswordMd5(MD5.toBase64(loginInfo.getPassword().getBytes()));
				}
			}
		} catch (Exception e1) {
		} finally {
	...
	public static void saveLoginInfo(LoginInfo loginInfo) {
		...
		try {
			...
			masterRecordStore = RecordStore.openRecordStore(LOGIN_RMS, true);
			masterRecordStore.addRecord(loginInfo.isSaveUserNameAndPassword() ? new byte[] {1} : new byte[] {0}, 0, 1);
			masterRecordStore.addRecord(loginInfo.isAutoLogin() ? new byte[] {1} : new byte[] {0}, 0, 1);
			
			if (loginInfo.isSaveUserNameAndPassword() || loginInfo.isAutoLogin()) {
				byte[] userNameBytes = loginInfo.getUserName().getBytes();
				masterRecordStore.addRecord(userNameBytes, 0, userNameBytes.length);
				
				byte[] passwordBytes = loginInfo.getPassword().getBytes();
				masterRecordStore.addRecord(passwordBytes, 0, passwordBytes.length);
			}
		} catch (Exception e1) {
		} finally {
			if (masterRecordStore != null) {
