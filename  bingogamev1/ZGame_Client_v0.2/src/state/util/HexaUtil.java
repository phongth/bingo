...
public class HexaUtil {
	 ...
	 * @param isTrim - Có loại bỏ các phần tử 0 ở 2 đầu của chuỗi hexa hay không
	 ...
	 * 
	 * @param value - Giá trị int cần chuyển đổi
	 * @param isTrim - Có loại bỏ các phần tử 0 ở 2 đầu của chuỗi hexa hay không
	 ...
	 * 
	 * @param byteArray - Mảng byte cần chuyển
	 * @param off - Index của phần tử đầu của đoạn các phần tử cần chuyển đổi trong mảng
	 * @param len - Số lượng các phần tử cần chuyển đổi
	 * @param isTrim - Có loại bỏ các phần tử 0 ở 2 đầu của chuỗi hexa hay không
	 * @return Chuỗi hexa là kết quả chuyển đổi của mảng byte
	 */
	public static String toHexaString(byte[] byteArray, int off, int len, boolean isTrim) {
		...
		for (int i = off; i < end; i++) {
			...
			if (isTrim && !isHaveAnyValue && high.equals("0") && low.equals("0")) {
				continue;
			}
	...
	private static String toChar(final int value) {
		...
		switch (value) {
			...
			case 14: return "E";
			case 15: return "F";
			default: throw new IllegalArgumentException("Value sai: " + value);
		}
	}
