...
public class Alert extends GameForm implements TimerListener {
  ...
  public Alert showCustomUIAlert(AlertListener listenner, Sprite[] customUISprites, String[] buttonLabels, byte buttonWidth, int alertId, byte alineType) {
    AlertInfo info = new AlertInfo();
  ...
  public Alert checkToShowQueueAlert() {
    if (GameGlobal.systemCanvas.isInTransforming() || GameGlobal.systemCanvas.isAlertShowing()) {
      return this;
    }
  ...
  private void loadImage() {
    try {
      bgImage = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/bg_popup.png");
      coverImage = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/pxiel_nen.png");
      coverImage = ImageUtil.joinAndCreateImages(coverImage, GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT, true);
      if (alertInfo.buttonWidth == BUTTON_WIDTH_SMALL) {
        buttonImage = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/button_nho.png");
        focusedButtonImage = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/button_nho_focus.png");
      } else {
        buttonImage = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/button_to.png");
        focusedButtonImage = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/button_to_focus.png");
      }

      if ((alertInfo.alertType == LOADING_FORM) || (alertInfo.alertType == LOADING_WITH_CANCEL_BUTTON_TYPE)) {
        loadingImage1 = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/loading_1.png");
        loadingImage2 = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/loading_2.png");
        ...
        loadingImage1 = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/loading_1.png");
        loadingImage2 = Image.createImage(GameConstants.CORE_IMAGE_FOLDER + "/loading_2.png");
        bgLoadingImage = Image.createImage(GameConstants.IMAGE_FOLDER + "/bg_loading.png");
      }
    } catch (IOException e) {
    }
    parentScreen = Image.createImage(GameConstants.SCREEN_WIDTH, GameConstants.SCREEN_HEIGHT);
  }
  ...
  public void doTask() {
    if (GameGlobal.systemCanvas.isAlertShowing()) {
      count++;
      if (alertInfo.alertType == LOADING_WITH_NO_BUTTON_TYPE && count >= alertInfo.alertTimeOut) {
        GameGlobal.systemCanvas.hideAlert();
        alertInfo.listenner.alertEventPerform(alertInfo.alertType, 0, alertInfo.alertId); // Khi timeout thì báo lại sự kiện ra ngoài
      }
    } else {
      destroy();
    }
  ...
  public void draw(Graphics g) {
    ...
    // Viết message
    if ((alertInfo.alertType != LOADING_FORM) && (alertInfo.alertType != LOADING_WITH_CANCEL_BUTTON_TYPE) && (alertInfo.alertType != LOADING_WITH_NO_BUTTON_TYPE)) {
      for (int i = 0; i < alertInfo.alertMessages.size(); i++) {
        vnText.drawString(g, String.valueOf(alertInfo.alertMessages.elementAt(i)), Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, alertY + messageY + 16 * i, GameConstants.TOP_HCENTER_ANCHOR);
      }
    }
    ...
    if ((alertInfo.alertType == LOADING_FORM) || (alertInfo.alertType == LOADING_WITH_CANCEL_BUTTON_TYPE)) {
      ...
      if (GameConstants.IS_240x320_SCREEN) {
        g.drawImage(logoImage, GameConstants.SCREEN_WIDTH / 2, 76, GameConstants.TOP_HCENTER_ANCHOR);
        vnText.drawString(g, "Đang tải...", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, 264, GameConstants.TOP_HCENTER_ANCHOR);
        x = 62;
        y = 247;
      } else {
        g.drawImage(logoImage, GameConstants.SCREEN_WIDTH / 2, 32, GameConstants.TOP_HCENTER_ANCHOR);
      ...
      if (GameConstants.IS_240x320_SCREEN) {
        if (alertInfo.alertMessages.size() > 0) {
          vnText.drawString(g, String.valueOf(alertInfo.alertMessages.elementAt(0)), Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, GameConstants.SCREEN_HEIGHT / 2 + 3, GameConstants.TOP_HCENTER_ANCHOR);
        }
        x = 62;
      } else {
        if (alertInfo.alertMessages.size() > 0) {
    ...
    } else if (alertInfo.alertType == INPUT_NUMBER_TYPE) { // Draw Input text field
      g.setColor(Color.WHITE_CODE);
      g.fillRect(GameConstants.SCREEN_WIDTH / 2 - 50, alertY + 15 + (alertInfo.alertMessages.size() - 2) * 20, 100, 18);
      vnText.drawString(g, String.valueOf(alertInfo.alertId), Color.BLUE_CODE, GameConstants.SCREEN_WIDTH / 2 - 48, alertY + 13 + (alertInfo.alertMessages.size() - 2) * 20, GameConstants.TOP_LEFT_ANCHOR);
      vnText.drawString(g, "Xóa", Color.WHITE_CODE, GameConstants.SCREEN_WIDTH - 10, alertY - 5 + alertInfo.alertMessages.size() * 20, GameConstants.TOP_RIGHT_ANCHOR);
  ...
  private void drawHorizontalAlineButton(Graphics g) {
    int leftX = (GameConstants.SCREEN_WIDTH - buttonImage.getWidth() * alertInfo.buttonLabels.length - (alertInfo.buttonLabels.length - 1) * BUTTON_DISTANCE_HORIZONTAL) / 2;
    int x = leftX;
    for (int i = 0; i < alertInfo.buttonLabels.length; i++) {
      if (i == buttonIndex) {
        g.drawImage(focusedButtonImage, x, alertY + 100, GameConstants.TOP_LEFT_ANCHOR);
      } else {
        g.drawImage(buttonImage, x, alertY + 100, GameConstants.TOP_LEFT_ANCHOR);
      }
      vnText.drawString(g, alertInfo.buttonLabels[i], Color.WHITE_CODE, x + buttonImage.getWidth() / 2, alertY + 100 + buttonImage.getHeight() / 2, GameConstants.CENTER_ANCHOR);
  ...
  private void drawVerticalAlineButton(Graphics g) {
    ...
    for (int i = 0; i < alertInfo.buttonLabels.length; i++) {
      if (i == buttonIndex) {
        g.drawImage(focusedButtonImage, GameConstants.SCREEN_WIDTH / 2, y, GameConstants.TOP_HCENTER_ANCHOR);
      } else {
        g.drawImage(buttonImage, GameConstants.SCREEN_WIDTH / 2, y, GameConstants.TOP_HCENTER_ANCHOR);
      }
      vnText.drawString(g, alertInfo.buttonLabels[i], Color.WHITE_CODE, GameConstants.SCREEN_WIDTH / 2, y + buttonImage.getHeight() / 2, GameConstants.CENTER_ANCHOR);
  ...
  public void setLoadingPercent(int loadingPercent) {
    if (loadingPercent >= 100) {
      if (parent != null) {
        parent.alertEventPerform(alertInfo.alertType, LOADING_DONE_EVENT, alertInfo.alertId);
      }
    }
  }
  ...
  public Alert setAlertTimeOut(int alertTimeOut) {
    this.alertInfo.alertTimeOut = alertTimeOut / GameGlobal.systemCanvas.getTimer().getDelay();
    return this;
  }
    ...
    if (alertInfo.customUISprites != null) {
      for (int i = 0; i < alertInfo.customUISprites.length; i++) {
        if (alertInfo.customUISprites[i].keyListener != null) {
          alertInfo.customUISprites[i].keyListener.keyPressed(alertInfo.customUISprites[i], keyCode);
        }
      }
    }
  }
  ...
  public void keyRepeated(int keyCode) {
    ...
    if (alertInfo.customUISprites != null) {
      for (int i = 0; i < alertInfo.customUISprites.length; i++) {
        if (alertInfo.customUISprites[i].keyListener != null) {
          alertInfo.customUISprites[i].keyListener.keyRepeated(alertInfo.customUISprites[i], keyCode);
        }
      }
    }
  }
  ...
  public void keyReleased(int keyCode) {
    if ((alertInfo.buttonLabels == null) || (alertInfo.buttonLabels.length == 0)) {
      return;
    }
    ...
    if (alertInfo.customUISprites != null) {
      for (int i = 0; i < alertInfo.customUISprites.length; i++) {
        if (alertInfo.customUISprites[i].keyListener != null) {
          alertInfo.customUISprites[i].keyListener.keyReleased(alertInfo.customUISprites[i], keyCode);
        }
      }
    }
    ...
    case Key.FIRE:
      ...
      switch (alertInfo.alertType) {
      case OK_TYPE:
        alertInfo.listenner.alertEventPerform(alertInfo.alertType, OK_BUTTON, alertInfo.alertId);
        break;
      case YES_NO_TYPE:
        switch (buttonIndex) {
        case 0:
          alertInfo.listenner.alertEventPerform(alertInfo.alertType, YES_BUTTON, alertInfo.alertId);
          break;
        case 1:
          alertInfo.listenner.alertEventPerform(alertInfo.alertType, NO_BUTTON, alertInfo.alertId);
          break;
        }
        break;
      case LOADING_WITH_CANCEL_BUTTON_TYPE:
        alertInfo.listenner.alertEventPerform(alertInfo.alertType, CANCEL_BUTTON, alertInfo.alertId);
        break;
      case CUSTOM_BUTTON_TYPE:
      case CUSTOM_UI_TYPE:
        alertInfo.listenner.alertEventPerform(alertInfo.alertType, buttonIndex, alertInfo.alertId);
        break;
      case INPUT_NUMBER_TYPE:
        alertInfo.listenner.alertEventPerform(alertInfo.alertType, buttonIndex, alertInfo.alertId);
        break;
    ...
    case Key.SOFT_RIGHT:
      ...
      if (str.length() < 2) {
        ...
        alertInfo.alertId = Integer.parseInt(str.substring(0, str.length() - 1));
      }
      break;
    default:
      if (alertInfo.alertType == INPUT_NUMBER_TYPE) {
        if ((Key.K_0 <= keyCode) && (keyCode <= Key.K_9)) {
          ...
          if (alertInfo.alertId == 0) {
            ...
            alertInfo.alertId = Integer.parseInt(alertInfo.alertId + String.valueOf(value));
          }
        }
      }
      break;
