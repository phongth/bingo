...
public class ListView extends Component {
	...
	public ListView(int numberOfColumn) {
		if (numberOfColumn < 1) {
			throw new IllegalArgumentException("ListView : numberOfColumn can not be smaller than 1");
		}
	...
	public ListView(int numberOfColumn, String[] columnTitle) {
		...
		if (columnTitle == null) {
			throw new IllegalArgumentException("ListView : coloumTitle can not be NULL");
		}
	...
	public void addRow(String[] rowData) {
		if (rowData == null) {
			throw new IllegalArgumentException("ListView : addRow : rowData can not be NULL");
		}
	...
	public void setColumnWidth(int cloumnIndex, int width) {
		if ((cloumnIndex < 0) || (cloumnIndex > columnWidths.length - 1)) {
			throw new IllegalArgumentException("ListView : setColumnWidth : cloumnIndex is invalid: " + cloumnIndex);
		}
		
		if (width < 0) {
			throw new IllegalArgumentException("ListView : setColumnWidth : width is invalid: " + width);
		}
	...
	public void setColumnWidths(int[] columnWidths) {
		if (columnWidths == null) {
			throw new IllegalArgumentException("ListView : setColumnWidths : columnWidths can not be NULL");
		}
      ...
      return;
		...
		if (isShowColumnTitle) {
			...
			for (int i = 0; i < columnTitles.length; i++) {
				titleFont.drawString(g, columnTitles[i], titleForceGroundColor, currentColumnX + columnWidths[i] / 2, realY + titleRowHeight / 2, GameConstants.CENTER_ANCHOR);
		...
		if (isFillBackGround) {
			...
			g.fillRect(realX, realY + dy, totalColumnWidth, numberOfRow * rowHeight);
		}
		
		if (hasBorder) {
			g.setColor(borderColor);
			g.drawRect(realX, realY + dy, totalColumnWidth, numberOfRow * rowHeight - 1);
		}
		...
		for (int i = 0; i < numberOfRow; i++) {
			...
			for (int j = 0; j < numberOfColumn; j++) {
				font.drawString(g, String.valueOf(data[j].elementAt(i)), forceGroundColor, currentColumnX + columnWidths[i] / 2, currentRowY + rowHeight / 2, GameConstants.CENTER_ANCHOR);
				currentColumnX += columnWidths[i];
			}
